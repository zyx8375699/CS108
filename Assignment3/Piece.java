...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		for(int i = 0; i < points.length; i++) {
			...
			topMost = Math.max(topMost, point.y);
		}
		width = rightMost + 1;
		height = topMost + 1;
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		TPoint rotatePoints[] = new TPoint[count];
		for(int i = 0; i < count; i++) {
			int newX = height - body[i].y - 1;
			int newY = body[i].x;
			rotatePoints[i] = new TPoint(newX, newY); 
		}
		return new Piece(rotatePoints);
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		TPoint[] otherBody = other.body;
		for(int i = 0; i < body.length; i++) {
			boolean canFind = false;
			for(int j = 0; j < otherBody.length; j++) {
				if(otherBody[j].equals(body[i])) {
					canFind = true;
					break;
				}
			}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece current = root;
		while(true) {
			current.next = current.computeNextRotation();
